...
// private methods
static gboolean cheese_pipeline_video_have_data_cb(GstElement *element, GstBuffer *buffer, GstPad *pad, gpointer user_data);
static void cheese_pipeline_video_lens_open(PipelineVideo *self);
static void cheese_pipeline_video_create_video(unsigned char *data, int width, int height);
...
{
  cheese_pipeline_video_lens_open(self);
  return;
}

static void
cheese_pipeline_video_lens_open(PipelineVideo *self)
{
...
cheese_pipeline_video_change_effect(gchar *effect, gpointer self)
{
  ...
  if (effect != NULL) {
    ...
    gst_bin_remove(GST_BIN(PIPELINE_VIDEO(self)->pipeline), priv->effect);

    g_print("changing to effect: %s\n", effect);
    priv->effect = gst_parse_bin_from_description(effect, TRUE, NULL);
    gst_bin_add(GST_BIN(PIPELINE_VIDEO(self)->pipeline), priv->effect);
...
cheese_pipeline_video_create(gchar *source_pipeline, PipelineVideo *self) {
  ...
  g_signal_connect(G_OBJECT(self->fakesink), "handoff", 
                   G_CALLBACK(cheese_pipeline_video_have_data_cb), self);

}
static gboolean
cheese_pipeline_video_have_data_cb(GstElement *element, GstBuffer *buffer, GstPad *pad, gpointer self)
{
  PipelineVideoPrivate *priv = PIPELINE_VIDEO_GET_PRIVATE(self);

  unsigned char *data_video = (unsigned char *)GST_BUFFER_DATA(buffer);
  if (priv->picture_requested) {
    ...
    cheese_pipeline_video_create_video(data_video, width, height);
...
void
cheese_pipeline_video_create_video(unsigned char *data, int width, int height)
{
  int i;
  gchar *filename = NULL;
  GnomeVFSURI *uri;
