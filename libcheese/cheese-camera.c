...
cheese_camera_set_camera_source (CheeseCamera *camera)
{
  ...
  g_signal_connect_swapped (decodebin, "pad-added",
      G_CALLBACK (cheese_camera_on_decodebin_pad_added), camera);
...
cheese_camera_set_new_caps (CheeseCamera *camera)
{
  ...
  if (!gst_caps_is_empty (caps))
  {
    ...
    /* If the selected caps are image/jpeg, video_source will convert them to
     * raw video internally. Therefore, camerabin should always use video/x-raw
     * regardless of the caps passed to video_source_filter. */
    caps = gst_caps_make_writable (caps);
    for (i = 0; i != gst_caps_get_size (caps); ++i)
    {
      gst_structure_set_name (gst_caps_get_structure (caps, i), "video/x-raw");
    }
